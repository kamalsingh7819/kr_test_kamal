...
function long_url($short_url) {
  ...
  if(0 === strpos($short_url, 'http:') || 0 === strpos($short_url, 'https:'))
    return $short_url;
  ...
  if(PROD())
    ...
    switch (strtolower($type)) {
      ...
      case "httpsurl(search-show":
        return 'https://' . KrConfig::$SUB_DOMAIN . '.' . KrConfig::$BASE_DOMAIN . '/on/demandware.store/' . KrConfig::$DEMANDWARE_ID . '/fr/Search-Show?cgid=' . $chunks[1];
        break;
...
function kr_spacer($xs = null, $sm = null, $md = null, $lg = null) {
  if(null === $xs) $xs = 0;
  if(null === $sm) $sm = $xs;
  if(null === $md) $md = $sm;
  if(null === $lg) $lg = $md;
